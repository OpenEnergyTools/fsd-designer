...
function findResourceRef(
  ...
  if (element.tagName === 'SCL') return null;

  const resource = element.ownerDocument.querySelector(
    `ProcessResource[name="${resourceName}"]`
  );
...
function lnClassFilter(srcRef: Element): string | undefined {
  const rscRef = findResourceRef(
    srcRef,
    srcRef.getAttribute('resourceName') ?? ''
  );
  ...
  return selector?.match('lnClass="(.*?)"')?.[1];
}
...
export default class FunctionEditor9030 extends LitElement {
  ...
  private renderProcResSelector(
    ...
    const nextRscName = srcRefs.find(
      (val, index) => index > i && val.getAttribute('resourceName') !== rscName
    );

    if (!rscName) return html`<th></th>`;
    ...
    if (i === 0) {
      return html`<th rowspan="${nextRscNameIndex || '1'}">
        <mwc-icon-button
          icon="link"
          @click="${() => {
    ...
    const isChanged =
      srcRefs[i].getAttribute('resourceName') !==
      srcRefs[i - 1].getAttribute('resourceName');

    return isChanged
      ? html`<th rowspan="${nextRscNameIndex || '1'}">
          <mwc-icon-button
            icon="link"
            @click="${() => {
              this.isSrcRefUpdate = true;
  ...
  private renderLNodeDetailContent(): TemplateResult {
    if (this.lNodeDetail === 'inputs') {
      const srcRefs = Array.from(
        ...
        const nameA = a.getAttribute('resourceName') ?? 'undefined';
        const nameB = b.getAttribute('resourceName') ?? 'undefined';
  ...
  private renderInputs(): TemplateResult {
    ...
    return html`${this.inputs.map(
      input =>
        html`<abbr class="input selectpane" title="${input.source}">
          <div
            class="${classMap({
              link: true,
              input: true,
  ...
  private renderOutputs(): TemplateResult {
    return html`${this.outputs.map(
      input =>
        html`<abbr class="input selectpane" title="${input.controlled}">
          <div
            class="${classMap({
              link: true,
              input: true,
